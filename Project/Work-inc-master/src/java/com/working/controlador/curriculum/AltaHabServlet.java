 ...
 * @author Hp
 */
@WebServlet(name = "AltaHabServlet", urlPatterns = {"/AltaHabServlet"})
public class AltaHabServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AltaHabServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AltaHabServlet at " + request.getContextPath() + "</h1>");
            try{
                iHabilidadDao xp = new HabilidadesDaoSQL();
                String escuela = request.getParameter("Habilidad");
                String correo = request.getParameter("correo");
                Habilidades exp = new Habilidades(0, correo, escuela);
                if(xp.altaHabilidades(exp)){
