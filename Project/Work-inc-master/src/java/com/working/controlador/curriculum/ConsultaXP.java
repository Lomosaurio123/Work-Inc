 ...
 * @author Hp
 */
@WebServlet(name = "ConsultaXP", urlPatterns = {"/ConsultaXP"})
public class ConsultaXP extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ConsultaXP</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ConsultaXP at " + request.getContextPath() + "</h1>");
            try{  
            HttpSession sesion = request.getSession();
            Usuario us = (Usuario)sesion.getAttribute("usuario");
            iExperienciaDao xp= new ExperienciaDaoSQL();
            List<Experiencia> exp=  xp.traeExperiencia(us.getCorreo());
            if(exp != null){
                request.setAttribute("usuarioCon", exp);
                request.getRequestDispatcher("Curriculum/Curriculum.jsp").forward(request, response);
            }else{
